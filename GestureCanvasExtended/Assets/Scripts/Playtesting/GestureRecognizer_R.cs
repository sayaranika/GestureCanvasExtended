...
public class GestureRecognizer_R : MonoBehaviour
{
    ...
    public void Initialize(Interaction i, PoseRecognizer p, PlaytestingManager play)
    {
        ...
        window = Target.GestureLength_R * 3;
    ...
    private void LateUpdate()
    {
        if (active == true)
        {
            if (detectedId != -1)
            {
                if(Target.expectedGestureId_R == detectedId)
                {
                    Debug.Log("4000N");

                    if (Target.isPose_L == true)
                    {
                        bool isRecognized = poseRecognizer.Recognize(Handedness.Left, null, Target.expectedPose_L, 
                            Target.isTransformConstraint_L, Target.isConditionSetToTrue);
                    }
                    else
                    {
            ...
            if (RealtimeDataStreamLength >= window && ReadDataFrame < RealtimeDataStream[window - 1].FrameNumber)
            {
                ...
                RealtimeDataStream.RemoveRange(0, Target.GestureLength_R);
