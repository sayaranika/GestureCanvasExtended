...
{
    public List<double> costEntry;
    public ListCost(List<double> myList)
    {
        costEntry = myList;
    }
}
...
public class Jackknife 
{
     ...
     */
    //public List<List<double>> cost = null;
    public List<ListCost> cost = null;

    /**
     * Vector of all gesture templates.
     */
    public List<JackknifeTemplate> templates;

    /**
         * The set of measures and features used in this instance
         * of Jackknife.
         */
    public JackknifeBlades Blades;
    ...
    public Jackknife(JackknifeBlades blades)
    {
        ...
        {
            throw new System.ArgumentException("!(blades.InnerProduct ^ blades.EuclideanDistance)");
        }
    ...
    public void Train(int gpsrN, int gpsrR, double beta)
    {
        ...
        //Debug.Log("600: template count " + templates.Count);
        ...
        //Debug.Log("600: created neg samples");
        //
        ...
        //Debug.Log("600: created pos samples");
        ...
        //Debug.Log("600: learnt rejection");
    }
    ...
    private double DTW(List<Vector> v1, List<Vector> v2)
    {
        ...
        else
        {
            for (int i = 0; i < v1.Count + 1; i++)
                for (int j = 0; j < v2.Count + 1; j++)
                    cost[i].costEntry[j] = double.PositiveInfinity;
        }


        cost[0].costEntry[0] = 0;
        ...
        for (int i = 1; i <= v1.Count; i++)
        {
            for (int j = Math.Max(1, i - Blades.Radius); j <= Math.Min(v2.Count, i + Blades.Radius); j++)
            {
                ...
                double minimum = Math.Min(Math.Min(cost[i - 1].costEntry[j],      // repeat v1 element
                                                   cost[i].costEntry[j - 1]),     // repeat v2 element
                                                   cost[i - 1].costEntry[j - 1]); // don't repeat either
                cost[i].costEntry[j] = minimum;
                if (Blades.InnerProduct)
                {
                    cost[i].costEntry[j] += 1 - v1[i - 1].Dot(v2[j - 1]);
                }
                else if (Blades.EuclideanDistance)
                {
                    cost[i].costEntry[j] += v1[i - 1].L2Norm2(v2[j - 1]);
                }
                else
                    throw new Exception("Should not get here!");
            }
        ...
        return cost[v1.Count].costEntry[v2.Count];
    }
}
